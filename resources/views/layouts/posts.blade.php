@foreach($user->photos as $photo)
                    <div class="post card p-3 post_card">
                      ...
                      <p>
                        @php
                        $like =$user->likes->where('photo_id',$photo->id )->where('like', '=', TRUE)->first()
                        @endphp

                        @if($like)
                        <!-- <a href="#" class="link-black text-sm mr-2"><i class="fas fa-share mr-1"></i> Share</a> -->
                        <a href="{{url('like/'.$photo->id.'/')}}" identifier="" class="link-black text-sm"><i class="fas fa-thumbs-up mr-1"></i> Like ({{$photo->likes->count()}})</a>
                        @else
                        <!-- <a href="#" class="link-black text-sm mr-2"><i class="fas fa-share mr-1"></i> Share</a> -->
                        <a href="{{url('like/'.$photo->id.'/')}}" identifier="" class="link-black text-sm"><i class="far fa-thumbs-up mr-1"></i> Like ({{$photo->likes->count()}})</a>
                        @endif
